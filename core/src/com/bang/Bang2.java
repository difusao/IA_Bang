...
public class Bang2 extends ApplicationAdapter implements InputProcessor {
	...
	boolean DiscoverWeights = false;
	double[][] rndWeights = new double[waveTotal][26];
	...
	private void Trainner(double inAngle, double inPower, double inObjDown, double inTargetX, double inTargetY, double inShotW, double inHight) {
		...
		if(wave == waveTotal) {
			...
			targetX = nnu.RamdomValues(50.0f, 90.0f);

			// Best by aprouch
			trainingSet1 = rna.Best(trainingSet1, Math.round((float)trainingSet1.getRows().size() * 0.5f), (inTargetX/100+0.05));
			...
			int iterations = rna.PerceptronMLSave(
					TransferFunctionType.SIGMOID,
					trainingSet1,
					new int[]{inputs.length, 6, outputs.length},
					FileDataset,
					FileNetwork + ".nnet",
					0.01f,
					0.2f,
		...
		if ( wave < waveTotal && (isCollideGround || isCollideTarget) && !test) {
			if (!collide) {
				...
				if(DiscoverWeights) {
					...
					//angle = (float)trainingSet1.getRows().get(0).getDesiredOutput()[0];
					//power = (float)trainingSet1.getRows().get(0).getDesiredOutput()[1] * 100;
	...
	public void create() {
		...
		//angle = 000.82887417f;
		//power = 000.31249511f * 100;

		//angle = 01.10968124866485600f;
		//power = 34.01906204223633000f;
		...
		/*
		trainingSet1.clear();
		trainingSet1.addRow(new DataSetRow(new double[]{0.90}, new double[]{0, 0}));

		double[] mlp = rna.MLP(
				trainingSet1.getRows().get(0),
				new int[]{1, 6, 2},
				0.01f,
				0.2f,
				FileNetwork + ".nnet"
		);
		System.out.println(Arrays.toString(mlp));
		*/
	}
